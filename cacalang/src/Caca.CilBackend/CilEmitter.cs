...
public sealed class CilEmitter
{
    ...
    public static IReadOnlyList<Diagnostic> Emit(
        ...
        out string program)
    {
        return Emit(unit, functions, out program, out _);
    }

    /// <summary>
    /// Same as <see cref="Emit(CompilationUnit, IReadOnlyDictionary{string, FunctionSymbol}, out string)"/>,
    /// but also returns the source-line map a step-by-step debugger needs to
    /// show the cacalang line behind the CIL instruction it's currently
    /// executing.
    /// </summary>
    /// <param name="lineMap">
    /// (byte offset, 1-based cacalang source line) pairs in ascending byte
    /// offset order, one per statement in the compiled program — empty when
    /// there are diagnostics. To find the source line for a running PC, take
    /// the LAST entry whose byte offset is &lt;= PC.
    /// </param>
    public static IReadOnlyList<Diagnostic> Emit(
        CompilationUnit unit,
        IReadOnlyDictionary<string, FunctionSymbol> functions,
        out string program,
        out IReadOnlyList<(int ByteOffset, int SourceLine)> lineMap)
    {
    ...
    {
        Line("; Generated by the cacalang compiler. Assemble and run with the CacaVM CLI.");
        Line("JMP __cil_start");
        Line("");
    ...
    private void EmitStringData(CompilationUnit unit)
    {
        ...
        foreach (var text in literals.Distinct())
        {
            ...
            _stringLabels[text] = (label, byteCount);
            Line($"{label}:");
            LineDb($"DB {DbStringLiteral(text)}", byteCount);
        }

        // Fixed labels the runtime print helpers need, regardless of whether
        // the program itself uses any string literals at all.
        Line("__str_true:"); LineDb($"DB {DbStringLiteral("true")}", Encoding.UTF8.GetByteCount("true"));
        Line("__str_false:"); LineDb($"DB {DbStringLiteral("false")}", Encoding.UTF8.GetByteCount("false"));
        Line("__str_minvalue:"); LineDb($"DB {DbStringLiteral(MinValueText)}", Encoding.UTF8.GetByteCount(MinValueText));
    }
    ...
    {
        _lineMap.Add((_byteOffset, statement.Location.Line));
    ...
    /// <summary>
    /// Bytes of CIL emitted so far, tracked alongside <see cref="_text"/> so
    /// <see cref="_lineMap"/> can record a real byte offset, not just a text
    /// line number. Every <see cref="Line"/> call is either a label (0 bytes,
    /// resolved by the assembler, never itself executed), a comment or blank
    /// line (0 bytes, stripped before assembly), or one fixed-width 6-byte
    /// instruction — <see cref="LineDb"/> is the one exception, for the
    /// variable-length <c>DB</c> directives <see cref="EmitStringData"/>
    /// emits, whose exact byte count the caller already knows.
    /// </summary>
    private int _byteOffset;

    /// <summary>
    /// (byte offset, source line) pairs in ascending byte-offset order, one
    /// per statement emitted — <see cref="EmitStatement"/> records the pair
    /// at entry, before emitting anything for that statement. A debugger
    /// maps a running PC back to the cacalang source line producing it by
    /// finding the LAST entry whose byte offset is &lt;= PC (later entries
    /// for the same offset — an outer block followed immediately by its
    /// first child statement — deliberately win, since the child is the
    /// more specific description of what's about to execute).
    /// </summary>
    private readonly List<(int ByteOffset, int SourceLine)> _lineMap = new();

    /// <summary>Emits one line of CIL text: a label, a comment, or exactly one 6-byte instruction.</summary>
    ...
    /// <summary>Emits a <c>DB</c> directive of a known, caller-computed byte length — see <see cref="_byteOffset"/>.</summary>
    private void LineDb(string text, int byteCount)
    {
        _text.AppendLine(text);
