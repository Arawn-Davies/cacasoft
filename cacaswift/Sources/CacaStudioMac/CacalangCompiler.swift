...
enum CacalangCompiler {
    ...
    /// Locates cacalang's own bundled samples directory (`cacalang/samples/
    /// *.caca` in the cacasoft monorepo) — the same files cacalang's own
    ...
    static func findSamplesDir() -> URL? {
        ...
        while dir.pathComponents.count > 1 {
            if dir.lastPathComponent == "cacaswift" {
    ...
    static func findCli() -> URL? {
        ...
        // Walk up from this source file to this package's root (cacaswift),
        // then look for a sibling `cacalang` checkout's built CLI.
        var dir = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
        while dir.pathComponents.count > 1 {
            if dir.lastPathComponent == "cacaswift" {
