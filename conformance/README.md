...
A corpus of test cases shared across every CacaVM engine implementation —
currently the C# reference implementation (`source/Caca.VM`) and the Swift
port (`mac/CacaStudioMac/Sources/CacaVMKit`) — expressed purely in terms of
observable behavior (stdout and errors), not internal register or memory
state, so any conforming engine can be checked against the same corpus via a
thin, engine-specific adapter. `source/Caca.VM.Tests` and
`mac/CacaStudioMac/Tests/CacaVMKitTests` each have one.
...
Each subdirectory of `cases/` is one test case:
