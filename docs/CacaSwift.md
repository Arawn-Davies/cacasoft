# cacaswift (CacaStudioMac)

A native macOS SwiftUI port of Caca Studio — the same CIL/cacalang IDE as
`CacaVM/source/Caca.VM.Studio` (WinForms), rebuilt from scratch against a
from-scratch Swift VM engine (`CacaVMKit`), not a wrapper around the C# one.
...
swift test    # runs CacaVMKitTests, including the shared conformance
              # corpus in CacaVM/conformance/cases (see docs/CIL-Conformance-Suite)
```
