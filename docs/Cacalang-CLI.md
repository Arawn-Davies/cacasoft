...
command line itself was wrong. See [`Cacalang-Diagnostics.md`](Cacalang-Diagnostics).
...
[`boot/`](Cacalang-Boot) turns into a GRUB-bootable ISO and boots in
...
[CacaVM](CacaVM), a small register-based
...
`--ref` names a .NET assembly that [`extern func`](Cacalang-Language#calling-net)
...
The repository ships one of these already; see [`Cacalang-Editor.md`](Cacalang-Editor).
