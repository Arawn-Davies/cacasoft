...
by `caca-langserver` (part of `cacalang/`) and `cil-langserver` (part of
`CacaVM/`).

1. Build and publish cacalang's server into `cacalang/`, where the
...
2. If you also want live errors for `.cil` files, publish CacaVM's server —
   anywhere, since there is no committed default path pointing at it:

   ```sh
   dotnet publish ../CacaVM/source/Caca.VM.LanguageServer -c Release -o artifacts/langserver
   ```
...
3. Install the extension:
   ...
   ```sh
   cd editors/vscode
   cp ../../../licenses/cacalang-vscode-LICENSE LICENSE
   npm install
...
task defined for CacaVM (or a multi-root workspace covering both
directories) can use it.
