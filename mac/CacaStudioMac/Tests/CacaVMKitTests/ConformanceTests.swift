...
/// /conformance/cases — see conformance/README.md. The C# engine
/// (source/Caca.VM.Tests) runs the exact same corpus via its own adapter;
/// a case only counts once both agree on it.
final class ConformanceTests: XCTestCase {
    private static var conformanceCasesDirectory: URL {
        // #filePath is this source file's own path at compile time; walk up
        // from it to the repo root (mac/CacaStudioMac/Tests/CacaVMKitTests/ -> repo root)
        // rather than relying on the test bundle's working directory, which
        // SPM does not guarantee points anywhere useful.
